...
function integrationSiteId($client){$siteId=(int)($client['site_id']?:($_GET['siteId']??0));if(!$siteId)integrationFail('INVALID_ARGUMENT','siteId不能为空');return $siteId;}
...
function integrationEnvelope($data,$definitionVersion='1.1',$freshnessSlaSeconds=86400,$capabilityAvailable=true){$id=integrationRequestId();header('Cache-Control: no-store');header('X-Request-Id: '.$id);$canonical=json_encode(integrationCanonical($data),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);return['requestId'=>$id,'traceId'=>$id,'systemId'=>'sanqi-overseas-site','sourceSiteId'=>'sanqi-overseas','contractVersion'=>'v'.INTEGRATION_CONTRACT_VERSION,'serviceVersion'=>INTEGRATION_SERVICE_VERSION,'sourceGeneratedAt'=>gmdate('c'),'dataAsOf'=>gmdate('c'),'definitionVersion'=>$definitionVersion,'freshnessSlaSeconds'=>$freshnessSlaSeconds,'isStale'=>false,'capabilityAvailable'=>$capabilityAvailable,'sourceDigest'=>'sha256:'.hash('sha256',$canonical?:'null'),'data'=>$data];}
